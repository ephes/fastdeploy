...
async def get_service_token(api_url: str, access_token: str, service_name: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            ...
            json={"service": service_name, "origin": "cli", "expiration_in_days": 1},
        )
        response.raise_for_status()
        data = response.json()
...
def print_step_status(steps: list, clear_previous: bool = False):
    ...
    for step in steps:
        ...
        status_icon = {"pending": "⏳", "running": "🔄", "success": "✅", "failure": "❌", "unknown": "❓"}.get(
            state, "❓"
        )
...
def get_password_from_1password(username: str) -> str | None:
...
async def main():
    ...
    try:
        ...
        print("FastDeploy Deployment Test")
        ...
        if not args.no_monitor:
            success = await monitor_deployment_with_steps(args.api_url, service_token, deployment_id, args.service)
